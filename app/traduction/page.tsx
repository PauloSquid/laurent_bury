...
function TraductionContent() {
  ...
  const triInitial: "genre" | "editeur" | "date" | "auteur" =
    triParam === "genre" ||
    triParam === "editeur" ||
    triParam === "date" ||
    triParam === "auteur"
      ? triParam
      : "genre";
  const [tri, setTri] = useState<"genre" | "editeur" | "date" | "auteur">(
    triInitial
  );
  ...
  const livresValides = useMemo(() => {
    ...
    return (livresData as Livre[])
      ...
      .sort((a, b) => {
        // Trier par priorité puis titre (pour tous les modes)
  ...
  const livresParGenre = useMemo(() => {
    ...
    const getGenreIndex = (genre: string): number => {
      const index = ordreGenres.indexOf(genre);
      return index >= 0 ? index : ordreGenres.length + 1; // Genres non listés après
    };

    // Trier les genres selon l'ordre spécifique, puis alphabétiquement pour les autres
    return Object.keys(groupes)
      .sort((a, b) => {
        const indexA = getGenreIndex(a);
        const indexB = getGenreIndex(b);
        if (indexA !== indexB) {
          return indexA - indexB;
        }
        // Si même index (genres non listés), trier alphabétiquement
      ...
      .reduce((acc, key) => {
        ...
        const extraireNomFamille = (nom: string | null): string => {
          ...
          return mots.length > 0
            ? mots[mots.length - 1].toLowerCase()
            : nom.toLowerCase();
        };
        livres.sort((a, b) => {
          ...
          if (nomFamilleA !== nomFamilleB) {
            ...
            return nomFamilleA.localeCompare(nomFamilleB, "fr", {
              sensitivity: "base",
            });
          }
  ...
  const livresParEditeur = useMemo(() => {
    ...
    // Trier les éditeurs par ordre alphabétique et les livres par nom de famille d'auteur
    return Object.keys(groupes)
      .sort()
      .reduce((acc, key) => {
        const livres = groupes[key];
